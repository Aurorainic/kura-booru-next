...
export default defineEventHandler(async (event) => {
  ...
  if (!body?.source_tag_id || !body?.target_tag_id) {
    throw createError({ statusCode: 400, statusMessage: 'source_tag_id and target_tag_id required' })
  }

  // B-P2-6: Self-merge check
  if (body.source_tag_id === body.target_tag_id) {
    throw createError({ statusCode: 400, statusMessage: '不能合并到自身' })
  }
  ...
  return db.transaction(async (tx) => {
    ...
    if (!source[0] || !target[0]) throw createError({ statusCode: 404, statusMessage: 'Tag not found' })
