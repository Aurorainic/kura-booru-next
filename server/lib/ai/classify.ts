...
/**
 * 动态构建 few-shot 样例段（agent 记忆锚点）：从 tag_knowledge 采样已确认分类
 * 注入 prompt，减少分类漂移。采样失败时优雅降级（无样例段）。
 */
...
export async function classifyTags(tagNames: string[]): Promise<TagClassification[]> {
  ...
  // agent 记忆优先：命中 tag_knowledge 直接用缓存（省 AI 调用）；尤其
  // manual 人工纠偏是硬约束——管理员改过的分类永远优先，避免模型翻案。
  ...
  for (const batch of chunk(uncachedNames, 25)) {
    ...
    try {
      ...
      // Keep only entries whose name matches an input tag (AI may hallucinate extras)
