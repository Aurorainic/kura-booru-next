...
<template>
  <div class="space-y-6 max-w-2xl">
    <PageHeader title="站点设置" subtitle="全局站点配置，变更立即生效。" />
    ...
    <PageHeader title="基础设施" subtitle="数据库和缓存连接配置。修改后需重启服务生效。" />
    ...
    <div class="dash-card !p-5 space-y-4">
      <div>
        ...
        <div class="flex gap-2">
          ...
          <button @click="testDb" :disabled="testingDb || !dbUrl" class="btn-ghost !text-xs !px-4 !py-2.5 !border !border-[var(--border-color)] !rounded-xl disabled:opacity-40">
            {{ testingDb ? '测试中…' : '测试连接' }}
          </button>
        </div>
      ...
      <div>
        ...
        <div class="flex gap-2">
          ...
          <button @click="testRedis" :disabled="testingRedis || !redisUrl" class="btn-ghost !text-xs !px-4 !py-2.5 !border !border-[var(--border-color)] !rounded-xl disabled:opacity-40">
