...
- 🗑️ **一键删除** — 管理面板直接删除插画，同步清理 S3 存储和数据库
...
│   ├── app/
│   │   ├── api/          #   REST 路由 (posts, tags, search, tasks, auth, auto-rating-rules, webhook)
│   │   ├── models/       #   SQLAlchemy 模型 (Post, Tag, PostTag, TagAlias, Admin, AutoRatingRule)
...
│   │   ├── layouts/      #   BaseLayout
│   │   ├── pages/        #   首页, 详情, 标签, 搜索, 登录, 管理后台
│   │   │   └── admin/    #     posts, auto-rating, import, password
...
```bash
cd infra && ./scripts/validate-env.sh dev    # 检查开发环境配置
cd infra && ./scripts/validate-env.sh prod   # 检查生产环境配置（严格）
```
...
```bash
cd infra && ./scripts/migrate-db.sh --dump-only                    # 仅导出
cd infra && ./scripts/migrate-db.sh --import-only dumps/xxx.sql    # 仅导入
cd infra && ./scripts/migrate-db.sh                                 # 交互式
```
