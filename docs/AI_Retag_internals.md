...
| 文件 | 职责 | 行数 | 关键函数/类 |
|------|------|------|-------------|
| `app/config.py` | 环境变量 | ~110 | `AI_PROVIDER_API_KEY`, `AI_PROVIDER_ENDPOINT`, `AI_PROVIDER_MODEL`, `ENABLE_AI_TAG_PROCESSING` |
...
| 文件 | 职责 |
|------|------|
| `src/lib/api.ts` | `Tag` 接口（`danbooru_name`, `translation`）、`fetchAdminTags()`, `updateAdminTag()`, `mergeTags()`, `reprocessTags()`, `updatePostTags()` |
| `src/pages/admin/tags.astro` | 标签管理页面（列表/编辑/合并/重处理） |
| `src/pages/admin/posts.astro` | 图片管理页面（新增标签列显示） |
| `src/pages/posts/[id].astro` | 详情页（管理员可添加/移除标签） |
| `src/components/TagBadge.astro` | 统一标签展示组件 |
...
**检查方法**：修改任何 `app/services/` 或 `app/tasks/` 文件时，grep 确认导入正确：
```bash
...
`api.ts` 导出的是 `getTagCategoryColor`，不是 `getTagCategoryColorClass`。`TagBadge.astro` 曾因此 build 失败（已修复，但其他新增代码可能再犯）。
