...
- 状态：已接受（2026-07-19）
- 关联：v0.9.0 规划文档 §B3 / §B4（已随仓库清理移除，见 git 历史）；技术验证：pg-boss（7/7 PASS，验证代码已随仓库清理移除，结论见本文）
- 决策输入：backend-audit §3（队列与异步现状）、§3.5（迁移面）、§8.2-2（Nitro 无后台任务抽象）（审计文档已随仓库清理移除，见 git 历史；结论见本文）
...
| B. pg-boss 收编 Node 侧 | 接口先行 + pg-boss 作为第二实现，收编 AI job、2 个 setInterval、后续 Node 内部任务；sidecar 桥保持 Redis | 技术验证全部通过；不重造重试/DLQ/cron；零新增容器（pg-boss 住在现有 PG） |
| C. 全收编 | 连 sidecar 桥与结果回取协议一起迁 pg-boss | 不可行：sidecar 消费不了（§3.5-1）；结果协议有 3 个外部消费方，契约风险高（§3.5-2）；Redis 仍删不掉（§3.5-3），收益不抵迁移面 |

## 验证结论（pg-boss 12.26.1，PG 16）

验证 demo 实测 7/7 PASS（验证代码已随仓库清理移除，结论见本文）：
...
实施注意（来自验证记录，代码已随仓库清理移除）：v12 worker 回调是批量签名 `async ([job]) =>`；DLQ 需先建；死信是**复制**（新 id、payload 保留、原 job 留 `failed`），关联要靠 data 里的业务 id。
