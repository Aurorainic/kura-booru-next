...
- 状态：已接受（2026-07-19）
- 关联：plan.md §F3 / 硬约束 4；spike：`spike/imgproxy/`（10/10 PASS）
- 决策输入：`docs/backend-audit-v0.9.0.md` §2.3、§6；`docs/frontend-audit-v0.9.0.md` §6.1、§6.2-1

## 背景

前端审计确认"无响应式图片"是明确短板：preview 只出一张图，没有 `srcset/sizes`，2x 屏手机和桌面拿同一张（frontend-audit §6.2-1）。plan.md §F3 给出两条路：imgproxy 通过则 srcset 近乎免费（100w–2000w 按需生成）；维持 sharp 则导入期预生成 2–3 档。
...
- 硬约束：plan.md「明确不做」写明**容器数不增**（现 4 个：web/worker/postgres/redis）。
- 数据规模：数百 posts、单机部署（backend-audit §5）。

## spike 结论（imgproxy 4.0.11 + MinIO）

`spike/imgproxy/` 实测 10/10 PASS：
...
4. **imgproxy 存档**：spike 已验证可行性并留下完整防 SSRF 配置模板与签名实现（`spike/imgproxy/`，含 v4 将 `IMGPROXY_ALLOWED_SOURCE_URL_PREFIXES` 改名为 `IMGPROXY_ALLOWED_SOURCES` 的坑——配错不报错不生效，启用前必须用 off-prefix 用例回归）。若未来约束放宽（容器数）或宽度档位需求爆炸（档案馆多档尺寸切换实时化），可按 spike README 直接启用，衔接方式为 `/i/` 路由保留、上游换成内部签名的 imgproxy（URL 形态不变，密钥不出 server）。
